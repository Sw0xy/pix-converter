...
function PxToRemCard() {
  const [result, setResult] = useState("");
  const handleConvert = () => {
    const px = document.querySelector(".px-input").value;
    const base = document.querySelector(".base-input").value;
    const rem = px / base;
    setResult(Number(rem));
  };

  return (
    <div className="mt-11 m-auto max-w-3xl p-3 md:p-6 bg-card rounded-lg w-full border border-secondary">
      ...
      <div className="flex mt-3 md:mt-6 items-center ">
        <label className="text-white mx-4">Rem</label>
        <input
          type="number"
          value={result}
          className="result-input form-control w-full ring-0
